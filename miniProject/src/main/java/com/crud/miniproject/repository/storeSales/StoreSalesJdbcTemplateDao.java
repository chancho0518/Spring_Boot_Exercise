...
public class StoreSalesJdbcTemplateDao implements StoreSalesRepository {
    ...
    static RowMapper<StoreSales> storeSalesRowMapper = ((rs, rsNum) ->
        new StoreSales(
                rs.getInt("id"),
    ...
    @Override
    public void updateSalesAmount(Integer storeId, Integer finalAmount) {
        jdbcTemplate.update("UPDATE store_sales SET amount=? WHERE id=?", finalAmount, storeId);
    }
}
